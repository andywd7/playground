...
jQuery('#js-toggle').attr('aria-pressed', 'false');
jQuery('#js-toggle').on('change', function () {

    jQuery('#js-toggle').attr('aria-pressed',
        jQuery('#js-toggle').attr('aria-pressed') == 'true' ? 'false' : 'true'
    );

    jQuery('.flyout__overlay').toggleClass('is-active');
    $body.toggleClass('navbar__push--right');
    jQuery('.is-active').not('.flyout__overlay').toggleClass('is-active');
});

jQuery('#toggle-nav > li').on('click', function () {
    jQuery(this).toggleClass('is-active');
    jQuery('#toggle-nav > li').not(this).removeClass('is-active');
    jQuery('.flyout__inner').animate({ scrollTop: jQuery(this).index() * 41 }, 700);
});

$body.on('click', '.flyout__overlay.is-active', function () {
    setTimeout(function() {
        jQuery('.is-active').removeClass('is-active');
        jQuery('.flyout__inner').animate({ scrollTop: 0 });
    }, 200);
    $body.removeClass('navbar__push--right');
    jQuery('#js-toggle').removeAttr('checked');
});
//}
